...
class RootCapabilityProvider(private val timeoutSeconds: Long = 15) {
    ...
    fun copyDirectory(sourcePath: String, destinationDir: File): RootCopyResult {
        return copyDirectory(sourcePath, destinationDir, null)
    }
    ...
    fun copyDirectory(
        ...
        val result = runSuCancellable("test -d $source && cp -a ${shellQuote(sourcePath + "/.")} $destination/", isCancelled)
    ...
    private fun copyFile(remotePath: String, destination: File, isCancelled: (() -> Boolean)? = null) {
        if (isCancelled?.invoke() == true) throw InterruptedException("Root copy cancelled")
    ...
    private fun runSuCancellable(command: String, isCancelled: (() -> Boolean)?): Result {
        return try {
            ...
            val stderr = StringBuilder()
            val stdoutThread = Thread { process.inputStream.bufferedReader().use(BufferedReader::readText).also(stdout::append) }.apply { start() }
            val stderrThread = Thread { process.errorStream.bufferedReader().use(BufferedReader::readText).also(stderr::append) }.apply { start() }
