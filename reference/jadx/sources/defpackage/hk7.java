...
public final class hk7 {
    ...
    public final void a(float f) {
        if (f < 0.0f) {
            c6.f("Damping ratio must be non-negative");
        } else {
            this.b = f;
            this.c = false;
        }
    ...
    public final void b(float f) {
        if (f <= 0.0f) {
            c6.f("Spring stiffness constant must be positive.");
        } else {
            this.a = Math.sqrt(f);
            this.c = false;
        }
    ...
    public final vr2 c(double d, double d2, long j) {
        double dSin;
        double dCos;
        if (!this.c) {
            if (this.i == Double.MAX_VALUE) {
                l0.e("Error: Final position of the spring must be set before the animation starts");
                return null;
            }
            double d3 = this.b;
            if (d3 > 1.0d) {
